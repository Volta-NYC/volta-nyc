...
import { PageHeader, SearchBar, Badge, Btn, Modal, Field, Input, Select, TextArea, Empty, StatCard, TagInput, useConfirm } from "@/components/members/ui";
import { subscribeProjects, createProject, updateProject, deleteProject, type Project, subscribeBusinesses, type Business } from "@/lib/members/storage";
...
export default function ProjectsPage() {
  ...
  useEffect(() => {
    ...
    const u2 = subscribeBusinesses(setBusinesses);
    return () => { u1(); u2(); };
  }, []);
  ...
  const handleSave = async () => {
    ...
    if (editing) await updateProject(editing.id, form as Partial<Project>);
    else await createProject(form as Omit<Project, "id" | "createdAt" | "updatedAt">);
    setModal(null);
  };
  ...
  return (
    <MembersLayout>
      <Dialog />
      <PageHeader title="Projects" subtitle={`${projects.length} total · ${projects.filter(p => p.status === "Active").length} active`}
        action={<Btn variant="primary" onClick={openCreate}>+ New Project</Btn>} />
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {filtered.map(p => (
          <div key={p.id} className="bg-[#1C1F26] border border-white/8 rounded-xl p-5 hover:border-white/15 transition-all">
            ...
            <div className="mb-3">
              <div className="flex justify-between text-xs text-white/30 mb-1"><span>Progress</span><span>{p.progress}</span></div>
              <div className="h-1.5 bg-white/8 rounded-full overflow-hidden">
                <div className="h-full bg-[#85CC17] rounded-full" style={{ width: `${PROGRESS_BAR[p.progress]}%` }} />
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-3 pt-3 border-t border-white/5">
              ...
              <Btn size="sm" variant="danger" onClick={() => ask(async () => deleteProject(p.id))}>Del</Btn>
            </div>
          </div>
        ))}
        {filtered.length === 0 && (
          <div className="col-span-3"><Empty message="No projects found." action={<Btn variant="primary" onClick={openCreate}>Create first project</Btn>} /></div>
        )}
      </div>
      <Modal open={modal !== null} onClose={() => setModal(null)} title={editing ? "Edit Project" : "New Project"}>
        <div className="grid grid-cols-2 gap-4 max-h-[65vh] overflow-y-auto pr-2">
          <div className="col-span-2"><Field label="Project Name" required><Input value={form.name} onChange={e => set("name", e.target.value)} /></Field></div>
          ...
          <div className="col-span-2"><Field label="Team Members"><TagInput values={form.teamMembers} onChange={v => set("teamMembers", v)} options={[]} /></Field></div>
