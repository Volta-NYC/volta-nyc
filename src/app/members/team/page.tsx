...
import { subscribeTeam, createTeamMember, updateTeamMember, deleteTeamMember, type TeamMember } from "@/lib/members/storage";
...
export default function TeamPage() {
  ...
  const handleSave = async () => {
    ...
    if (editing) await updateTeamMember(editing.id, form as Partial<TeamMember>);
    else await createTeamMember(form as Omit<TeamMember, "id" | "createdAt">);
    setModal(null);
  };
  ...
  return (
    <MembersLayout>
      ...
      <PageHeader title="Team Directory"
        subtitle={`${active.length} active · ${team.filter(m => m.role === "Team Lead").length} team leads`}
        action={<Btn variant="primary" onClick={openCreate}>+ Add Member</Btn>} />
      ...
      <Table cols={["Name", "School", "Role", "Division(s)", "Status", "Slack", "Actions"]}
        rows={filtered.map(m => [
          ...
          <div key="ac" className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <Btn size="sm" variant="danger" onClick={() => ask(async () => deleteTeamMember(m.id))}>Del</Btn>
          </div>,
        ])} />
      {filtered.length === 0 && <Empty message="No team members." action={<Btn variant="primary" onClick={openCreate}>Add first member</Btn>} />}
      <Modal open={modal !== null} onClose={() => setModal(null)} title={editing ? "Edit Member" : "New Member"}>
        <div className="grid grid-cols-2 gap-4 max-h-[65vh] overflow-y-auto pr-2">
          ...
          <Field label="Pod / Team"><Input value={form.pod} onChange={e => set("pod", e.target.value)} /></Field>
          <Field label="Join Date"><Input type="date" value={form.joinDate} onChange={e => set("joinDate", e.target.value)} /></Field>
          <div className="col-span-2"><Field label="Divisions"><TagInput values={form.divisions} onChange={v => set("divisions", v)} options={DIVISIONS} /></Field></div>
          <div className="col-span-2"><Field label="Skills"><TagInput values={form.skills} onChange={v => set("skills", v)} options={SKILL_OPTIONS} /></Field></div>
