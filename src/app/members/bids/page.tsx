...
import { subscribeBIDs, createBID, updateBID, deleteBID, type BID } from "@/lib/members/storage";
...
export default function BIDTrackerPage() {
  ...
  const openEdit = (bid: BID) => {
    ...
    setEditing(bid); setModal("edit");
  };
  const handleSave = async () => {
    if (!form.name.trim()) return;
    if (editing) await updateBID(editing.id, form as Partial<BID>);
    else await createBID(form as Omit<BID, "id" | "createdAt" | "updatedAt">);
    setModal(null);
  };
  ...
  const filtered = bids.filter((b) => {
    ...
    return (!search || b.name.toLowerCase().includes(q) || b.neighborhood.toLowerCase().includes(q) || b.contactName.toLowerCase().includes(q))
  ...
  return (
    <MembersLayout>
      <Dialog />
      <PageHeader title="BID Tracker" subtitle={`${stats.total} BIDs · ${stats.active} active · ${stats.pipeline} in pipeline`}
        action={<Btn variant="primary" onClick={openCreate}>+ New BID</Btn>} />
      ...
      <Table cols={["BID Name", "Status", "Neighborhood", "Contact", "Next Action", "Priority", "Actions"]}
        rows={filtered.map(b => [
          ...
          <div key="a" className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <Btn size="sm" variant="danger" onClick={() => handleDelete(b.id)}>Del</Btn>
          </div>,
        ])} />
      {filtered.length === 0 && <Empty message="No BIDs match your filters." action={<Btn variant="primary" onClick={openCreate}>Add first BID</Btn>} />}
      <Modal open={modal !== null} onClose={() => setModal(null)} title={modal === "create" ? "New BID" : "Edit BID"}>
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Neighborhood"><Input value={form.neighborhood} onChange={e => set("neighborhood", e.target.value)} /></Field>
          ...
          <div className="col-span-2"><Field label="Next Action"><Input value={form.nextAction} onChange={e => set("nextAction", e.target.value)} /></Field></div>
